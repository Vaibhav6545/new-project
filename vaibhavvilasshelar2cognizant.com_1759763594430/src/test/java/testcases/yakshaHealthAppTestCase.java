...
public class yakshaHealthAppTestCase extends AppTestBase
{
	...
	public void fillTheTextFieldInAddNewPatientPopup() throws Exception {
		...
		Assert.assertTrue(yakshaHealthAppInstance.findTheTextFieldInAddNewPatientPopup(), "Required Textfiels are not present");
	...
	public void verifyEnteredValueIsPresent() throws Exception {
		...
		Assert.assertEquals(yakshaHealthAppInstance.verifyFirstNameIsPresent(expectedData), expectedData.get("firstName"));
		Assert.assertEquals(yakshaHealthAppInstance.verifyMiddleNameIsPresent(expectedData), expectedData.get("middleName"));
		Assert.assertEquals(yakshaHealthAppInstance.verifyLastNameIsPresent(expectedData), expectedData.get("lastName"));
		Assert.assertEquals(yakshaHealthAppInstance.verifyAgeValueIsPresent(expectedData), expectedData.get("age"));
		Assert.assertEquals(yakshaHealthAppInstance.verifyContactNumberIsPresent(expectedData), expectedData.get("contact"));
	...
	@Test(priority = 8, groups = {"sanity"}, description="Verify that the dispensary arrow is expanding and submodules are displayed")
